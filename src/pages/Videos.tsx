...
function VideoCard({ video, onOpen }: { video: VideoItem; onOpen: (v: VideoItem) => void }) {
  ...
  return (
    <div className="group cursor-pointer" onClick={() => onOpen(video)}>
      <div className={`relative ${isPortrait ? 'aspect-[9/16]' : 'aspect-video'} rounded-xl overflow-hidden bg-stone-900`}>
        {video.thumbnail && (
          <img
            src={video.thumbnail}
            alt={video.title}
        ...
        <span className="absolute top-3 left-3 px-2 py-1 bg-stone-900/70 backdrop-blur-sm text-white text-[10px] uppercase tracking-wider rounded-xl">
          {video.category}
        </span>
      ...
      {natureVideos.length > 0 && (
        <section className="py-20 bg-stone-50">
          <div className="container-lux">
            ...
            {/* Note about videos */}
            <div className="mt-16 text-center bg-white p-8 rounded-xl">
